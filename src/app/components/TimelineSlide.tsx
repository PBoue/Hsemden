  ...
  const phases = [
    {
      ...
      deliverables: 'Problem-Backlog, Anforderungskatalog, Kontextinventar, Literatur-Review',
    },
    {
      id: '2.2',
      period: 'M 4-30',
      title: 'Konzeption, Prototyping, Validierung & Felderprobung',
      subtitle: 'Parallel verlaufende Design-Thinking-Zyklen (Design Sprints)',
      ...
      deliverables: 'Prototypen, Designsystem, Validierungsbericht, Architektur-Blueprint',
    ...
    },
    {
      ...
      deliverables: 'Implementierungsplan, Transfer-Packages, Publikationen',
    },
  ];
      ...
      <div className="max-w-7xl mx-auto space-y-12 relative">
        ...
        >
          <div className="inline-block px-4 py-2 bg-muted rounded">
            <span className="text-sm font-medium tracking-wide text-muted-foreground">09 — TIMELINE</span>
          </div>
          ...
          <div className="bg-card border border-border rounded-lg p-6">
            ...
            <div className="flex mb-4 text-xs text-muted-foreground font-medium">
              ...
              <div className="flex-1 flex">
                {[0, 6, 12, 18, 24, 30, 36, 42].map((month) => (
              ...
              {ganttPhases.map((phase, idx) => (
                <div key={idx} className="flex items-center gap-4">
                  <div className="w-64 flex-shrink-0">
                    <div className={`text-sm font-semibold ${phase.textColor}`}>
                      {phase.name}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      M {phase.start}–{phase.start + phase.duration - 1}
                    </div>
                  </div>
            ...
            <div className="mt-6 pt-6 border-t border-border">
              <p className="text-sm text-muted-foreground italic">
                <span className="font-semibold text-foreground">Design-Thinking-Parallelität:</span> Die mittlere Phase (M 4-30) umfasst iterative Design-Zyklen, in denen Konzeption, Prototyping und Validierung nicht sequentiell, sondern in parallelen Sprints ablaufen.
              </p>
            </div>
          </div>
        </motion.div>
        ...
        >
          ...
          {phases.map((phase, idx) => (
            <div key={phase.id} className={`space-y-4 pl-6 border-l-4 ${phase.color}`}>
              ...
              <div className="bg-card border border-border rounded-lg p-6">
                ...
                {phase.note && (
                  <div className="mt-6 pt-6 border-t border-border">
                    <p className="text-sm text-muted-foreground italic">
                      <span className="font-semibold text-foreground">→</span> {phase.note}
                    </p>
                  </div>
                )}
              </div>
